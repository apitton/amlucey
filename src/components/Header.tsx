...
const navBarLinks = [{location: '/', title: 'Home'}, {location: '/competence', title: 'Experience'}, {location: '/projects', title: 'Profile'}, {location: '/contact', title: 'Contact'}]
        ...
        <div className="sticky top-0 mx-auto bg-sky-700/80 h-20 text-white z-20">
            <Navbar className="flex justify-between w-3/4 mx-auto">
                ...
                <div className="flex gap-3 align-bottom">
                    {navBarLinks.map((link)=><Link onClick={()=>handleClick(link.location)} key={link.location} to={link.location} smooth={true} duration={500} className={`nav-link ${link.location==currentNav?"active":""}`}>{link.title}</Link>)}                    
